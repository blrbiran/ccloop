...
- **E3（Web 面板）已有定稿 spec ＋ 已过两席外审的实施计划，本体仍一行未写**；
  🆕 **两条开工前置现已【全部解除】**：correction seam 人 2026-09-10 授权且已落地；
  框架选型那条**早就在台账里**（`kind: dependency`／`scope: repo`，含服务端渲染 HTML 备选）。
  ...
  ⇒ 🔴 **记法（本仓库也用得上）**：*** **一条「现测 X 为零／不存在」的断言，一旦被写进【那件让它不再为零的产物】里，就会永久自证。引用前现跑。** ***
...
（更正 §五那句已知为假的「React 零命中」，并把两条新记法**折进已有的第 4、10 条**，
**一个编号项都没有新增** —— 规矩是人 2026-09-02 定的「关于 Orca 的章节不能无限增加下去」）。
上一次是 run `orca-dev-30fa920f` 于同日更新（当天两次：先整节重写，后就地补三处）。
⚠️ **第二次是【就地定点编辑】，不是整节重写** —— 规矩是「本节不无限增长」，所以两条新记法
**折进了第 7、10 条**，一个编号项都没有新增。
