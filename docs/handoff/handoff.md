...
- 🆕 **E3（Web 面板）在做（最近一轮 2026-09-15，Orca run `orca-dev-bad904b1`）**：
  前置一刀让 correction 的时钟可注入、面板与 CLI 共用**唯一**的行构造函数（人具名授权改既有生产代码）；
  **Task 0–5 已实施并各自过审**（`web/` workspace ＋ express ＋ `orca panel` 骨架；reviews 表自带一把 mkdir 锁、
  自己的拒绝码与显式 mode；安全边界 —— `--by` 必填、token 常时比较、非回环绑定要第二句确认；
  静态文件走精确文件名 Map；`GET /api/metrics` 每次请求重算 E2 的闸门），**Task 6–9 未做**，面板仍然只记、不闭环。
...
（§五 E3 那一条改写成「Task 0–5 已实施过审、Task 6–9 未做」；三条新实测**折进已有的第 3、4、11 条**，
**一个编号项都没有新增** —— 规矩是人 2026-09-02 定的「关于 Orca 的章节不能无限增加下去」）。
上一次是 run `orca-dev-5d7759dc` 于 2026-09-15 更新（E3 改写成「Task 0–3 已实施过审」，三条实测折进第 4、6 条）；
再上一次是 run `orca-dev-6354277a` 于 2026-09-11 更新（E3 改写成「已开工」，两条实测折进第 6、11 条）。
写入前**现测**本仓库：在 `main`、`git status --porcelain` 输出为空、
*** **`git ls-remote` 与本地【不同点】——本地领先两笔（主题行 `docs(handoff): roll the Orca section forward -- E3 has started, and two readings that were wrong`
与 `docs(handoff): roll the Orca section forward -- three more panel tasks, and three measurements that belong here`），都是 Orca 那条线前两轮写的，至今未推。** ***
