...
2. **已知 flake 债（刻意未修）——现为 7 个，不是 5 个**：
   ...
   - **【2026-07-29 新增，第 7 个】** `tests/controller/runLoop.integration.test.ts > runLoop > records retained cleanupStatus in execution recovery when cleanup fails`。症状：全套件并行负载下偶发失败；隔离复跑通过，其后全套件**连过两次 431/431**。**发现于债 4 的 Task 1 收口验证跑，当次提交是纯注释改动（4 行），因果上不可能造成它。** 与第 4–5 个的 `BUDGET_EXHAUSTED_REASON` 家族**无关**（它断言的是 cleanup 失败时的 `cleanupStatus` 保留，不是预算），也不是第 6 个。未做归因，未修。

   **给实施者与评审员**：在债 4 分支上跑全套件时，这三条具名失败**可以**出现且**不构成**新缺陷——
   `runLoop.integration.test.ts > treats execute timeout with no adapter result as exhausted even if files changed in the worktree`（断言期望 `'runtime or token budget exhausted'`、实得 `'execute phase exceeded per-attempt ti…'`）、上面第 6 个、以及上面第 7 个。
