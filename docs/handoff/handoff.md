...
3. *** **「绿」的一种坏法：判据对、变异也对，但两者之间隔着一条【时间上跑得比它慢】的断言。** ***
   ...
   🆕 **同一形状的 HTTP 版（Orca 2026-09-15 实测）**：*** **用 `fetch` 测路径穿越，测到的是 URL 解析器，不是服务器。** ***
   WHATWG URL 在【客户端】就把 `.`、`..` 以及百分号编码的 `%2e%2e` 折叠掉 ⇒ 发出去的已经是 `/etc/passwd`。
   ⇒ **走 `node:http` 发原始 path，并配一条正向对照**（一个「规范化后会命中」的拼写必须 404，而它的规范形式必须 200）。
   🆕 **同族第三种（Orca 2026-09-16 实测，run `orca-dev-5d5c8055`）**：*** **「什么都没发生」没法轮询到完成。** ***
   一条判据在响应回来后**立刻**读一次「0 行」，而被测的写入是 fire-and-forget、约 500 ms 后才落盘 ⇒ 让它「顺手多写一行」的变异**照绿 8/8**。
   ⇒ **「等它出现」可以轮询到成立；「确认它没出现」只能观测一个有界窗口，窗口从真实延迟来源（锁预算＋实测落盘时间）算出来。** 改完复跑 3/3 见红。
4. *** **一条判据可以在真实世界里造成它所测试的那个危害。** ***
   ...
   **变异跑（守卫被删掉）时，它会真的在所有网卡上起一个能读写使用者全局数据的服务。**
   ...
   Orca 实测：外审已把 `0.0.0.0` 从变异列清掉，计划照做了 —— 然后写了一条**省略绑定地址**的判据，
   而 `listen(port, undefined)` **绑所有网卡**。⇒ 跑「删掉绑定守卫」那条变异时，它会**真的把服务发布出去**。
   ⇒ *** **那一问要连【夹具里省略的参数展开成什么】一起问，不能只看变异列。** ***
   🆕 **2026-09-15 同一族的第三种，这次是【判据留下的东西】**：Orca 一条判据起真进程跑 CLI，
   *** **vitest 的 20 秒超时把判据判红，却不杀那个子进程** *** —— 跑两条变异时各留下一个还活着的 `tsx`
   （只绑回环，变异席手工杀掉并如实上报）。
   ⇒ **起真进程的判据要有显式 teardown；「判据红了」不等于「它起的东西没了」。**
   🆕 **2026-09-15 修好并被看见修好（Orca run `orca-dev-bad904b1`）**：*** **杀掉 `tsx` 的 pid 不等于杀掉在监听的那个进程** ***
   —— tsx 4.23.13 在【子】node 进程里跑脚本，只杀父进程会留下孤儿。修法是 detached spawn ＋ 对负 pid 发信号杀整个进程组 ＋ 期限短于测试超时；
   证明是变异席在「守卫被删掉」那两次跑里清点进程：**残留 0 个**。⇒ **teardown 的证据是【守卫被删掉时】的进程清点，不是正常路径上的绿。**
   🆕 **2026-09-17 同族的第四种，这次是【只读探针】（Orca run `orca-dev-bd5f202b` 实测）**：*** **只读打开 SQLite 不等于零触碰。** ***
   一席 spike 以 `?mode=ro` 打开人的 opencode 数据库（WAL 模式），**`-shm` 文件的 mtime 被改动**（内容未写）；改 `immutable=1` 不再触碰，但可能读不到尚在 WAL 里的新行。
   ⇒ **「只读」要问到文件系统层：探针会不会更新人的数据文件的任何元数据。**
...
6. *** **计划／文档里写下「应当红／应当绿」，就是写下一条【预言】，而预言会是假的。** ***
   ...
   🆕 **2026-09-11 同形的另一种（Orca run `orca-dev-6354277a` 实测）**：*** **「已就地更正」也是一条预言。** ***
   Orca 上一轮的处置节写着三处「已在上文就地更正」，开工扫描现测**三处都没落地** —— 读处置节的人会以为正文已经对了。
   ⇒ **引用「已修」之前，去被修的那一行看一眼。**
   🆕 **2026-09-15 两条同族的（Orca run `orca-dev-5d7759dc` 实测）**：
   ① *** **断言【形状】的那一条，在「换成一个形状合法的固定值」的变异下永远绿。** *** 实测：token 判据里
   `/^[0-9a-f]{64}$/` 在 `mintToken` 被改成一个写死的 64 位十六进制串之后**照绿**，红的是「两次 mint 不相等」那条。
   ⇒ **断言形状 ≠ 断言不可猜；每条形状断言旁边要有一条【值】的断言。**
   ② *** **计划里的代码块会 import 【未来】。** *** 实测：某 Task 的 `server.ts` import 了两个要在后面两个 Task 才存在的模块，
   照抄会让 `tsc` 直接红、整条 verify 停在这一刀。
   ⇒ **开工扫描加一条机械检查：本 Task 要写的每个 import，现在存不存在。**
   🆕 **2026-09-17 同族（Orca run `orca-dev-bd5f202b` 实测）**：*** **一份 spec 的正文可以和它自己的实测表矛盾，而作者自审看不出来。** ***
   正文写「不需要窗口大小」，同一份 spec 的表里 Codex 的窗口是 258,400（小于正文的 330K 阈值 ⇒ 阈值永不触发）；换「挑错席」重读才抓到。
   ⇒ **写完文档，拿每条论断去对文档自己的实测表。**
7. *** **一条变异要能红，先问「被删掉的那件事，是不是【唯一】在做它的那件事」。** ***
   ...
   Orca 为「两个模式共用同一处字面量」找落点时普查了一遍，发现**一整侧根本没有判据钉住那一行的内容**；
   已有那条「id 是所存行的派生」**在任何『行与 id 一起改』的变异下都照绿** —— 它钉自洽，不钉内容。**补了一条判据才有落点。**
   🆕 **第四个用法（Orca 2026-09-16 实测）**：*** **变异「落上去了」（源码 sha 变了），却可能对【产物】毫无作用。** ***
   改构建配置 `assetsDir` 的变异照绿 —— 那次构建根本没有非入口资源，产物逐字节不变。换成真会产生子目录的改法立刻见红。
   ⇒ **改构建配置／生成器的变异，要量产物，不只量源码 hash；照绿时先问「产物变了没有」，再问「是不是缺判据」。**
   🆕 **第五个用法，也是这条问句第一次【在落地之前】用（Orca 2026-09-16 实测，run `orca-dev-cbe8dda3`）**：
   一条 parked 裁决同时点名两件修法，控制器先做了第一件，然后**在落地第二件之前**问「删掉它自己的那条变异，能红吗？」——
   答不上来就去量：写一个探针把 fetch 换成**已决议的 Promise**（最可能让框架把两次状态更新批进同一次提交的情形），
   现测那个组件**仍然每次都卸载**（`same-node: false`、值为空）⇒ 第二件修法**在每条走得到的路径上都是冗余的，没落地**。
   ⇒ *** **前四个用法都是「变异照绿了，回头查」；这一个是「还没写，先问」。** *** **代价从一轮变异降到一个探针。**
   🆕 **第六个用法，也是「先问」的第二次，而这次答案是【红不了】（Orca 2026-09-16 实测，run `orca-dev-4ccc0a0b`）**：
   修一个 Host 头解析缺陷时，控制器一口气写了**两道**守卫（一道限定括号里的字符集，一道要求必须含冒号）。
   落地前先问「删掉字符类【它自己】的变异能红吗」，答不上来 ⇒ 去量：换成宽松字符类后 **11 条判据全绿**
   （sha 前后不同、变异行读回确认）。根因是该函数的返回值**只会被拿去做精确匹配**，
   两种畸形写法都不含冒号 ⇒ **冒号那一道单独就做完了全部的事，字符类删掉。**
   ⇒ *** **两次兑现连起来的记法：「先问再落地」的代价是一次探针，「落地了再回头查」的代价是一整轮变异。** ***
   🆕 **第七个用法，这次是【终审】替计划问的（Orca 2026-09-17 实测，run `orca-dev-5e5985bc`）**：计划的变异表 29 条、条条见红，
   终审仍找到一行新代码 **`finally { inFlight.delete(key) }` 删成「只在成功时删」⇒ 全部 132 条面板判据照绿**，而它要防的洞原样回来。
...
10. 🆕 *** **「期望红在 X」写得【不全】，和写得【假】是同一类危害。** ***
    ...
    *** **然后回去改没坏的代码** *** —— 与第 6 条引出的是同一个错误动作。
    ...
    🆕 ⚠️ *** **写下这一条的人，在同一个会话里【当天就又违反了它一次】。** *** Orca 实测：一条变异预言红 2 条、实际红 3 条，
    多出来的那条走的正是同一个被删分支。⇒ *** **知道一条记法不等于会用它。普查要写成【可跑的 grep】，不许靠脑补。** ***
    🆕 🔴 **2026-09-10 又一轮：Orca 写了 16 条预言，被两席外审现测【错了 5 条】（31%）—— 而且 grep 没兜住。**
    最值得记的那条：它以为两处 caveat 是同一样东西，*** **grep 搜关键词会同时命中两处、看起来正好印证预言** ***，
    实际它们来自**两个不同的字段**。
    ⇒ *** **记法补第三维**：除了「X 之前有没有别的断言先炸」（防假）＋「被删那行还有谁在走」（防不全），
    ...
    *** **而判据层的自审在【计划阶段根本没法真做】，因为被测代码还不存在。**
    ...
    🆕 **2026-09-17 那一轮按上面的记法写了 33 条预言，31 条精确、2 条不全（都是真红）**（Orca run `orca-dev-5e5985bc`）。
    两条漏算各一个原因：一条判据按字节比整段输出（「字面量从哪来」没问到）；另一条是**写完预言之后才新加的判据** ——
    ⇒ *** **加判据时要回头扫一遍既有预言：新判据会不会也被旧变异打红。** ***
    🆕 **反方向（Orca 2026-09-17 实测，run `orca-dev-18c77f3e`）**：*** **在上游修掉根因，会让下游一道旧守卫变冗余、钉它的旧变异从红变绿。** ***
    修 `readReviews` 之前先回扫旧变异表，发现 M30（删下游同形过滤）修后必绿 ⇒ 把那道守卫一并删掉，C22 改由新变异钉住，两条新变异预言都精确。
    ⇒ **改生产代码前也要回扫：旧变异里有没有哪条会因为这次修改【变绿】。**
11. 🆕 *** **守着那条铁律的扫描器，自己带着假阳性被交付了。** ***
    ...
    *** **「在本文上跑出 BAD_COUNT = 0」什么都不证明 —— 一个恒返回 0 的扫描器给同样的输出。** ***
    ...
    🆕 **2026-09-15 同族、方向再反（Orca run `orca-dev-bad904b1`，实测两次）**：*** **在工具调用里写 NUL 类转义，落盘的是【裸字节】。** ***
    ① 实施者的 Write 把源码里本该是转义的分隔符写成两个裸 NUL ⇒ **git 把整个源文件判成二进制，评审包里只剩一行 `Binary files differ`** ——
    评审员根本看不到那个文件，「通过」照样出得来；是控制器看 diffstat 里的 `Bin` 才抓到的。
    ② 控制器往台账记这件事时，heredoc 里原样写了那个转义 ⇒ Bash 工具以「含隐藏控制字符」拒收。
    ⇒ *** **brief／heredoc／Write 里不直接写这类转义，用文字或 `chr()` 构造；写完对碰过的文件做字节扫描；收到评审包先看 diffstat 有没有 `Bin`。** ***
    🆕 **2026-09-16 又落盘三次（累计五次，Orca run `orca-dev-5d5c8055`）**：实施者的 Write 两次、实施者自己的报告里一次 ——
    *** **每一次都是【每次编辑后字节扫描】抓住的，「不许写这类转义」的禁令一次都没拦住。** *** ⇒ **扫描是护栏，禁令只是提醒。**
    🆕 **2026-09-16 同一条的【判据版】（Orca run `orca-dev-4ccc0a0b` 实测）**：
    *** **一条「写下来那天就是绿的」不变量守卫判据，必须自带【必抓样本】，否则它和恒返回空的桩没有区别。** ***
    Orca 写了一条钉「某依赖链只出现在 dev 位置」的判据 —— 今天本来就成立。先把谓词写成 `return []` 的桩来验：
    **必抓样本红了，而【真实仓库那条判据照绿】** —— 那正是它要防的假绿。
    ⇒ **必抓与必不抓两组样本之外，还要有「对真实输入」和「对必抓夹具」两条判据；只有前者等于没有。**
    🆕 **同一轮的第二条，方向是【工具在骗你】**：*** **`npm audit` 的文字报告会把 high 与 critical 折叠掉。** ***
    现测：文字报告只列 2 条 advisory（都是 moderate），它自己的汇总行却写 `5 (3 moderate, 1 high, 1 critical)`。
    ⇒ *** **只读文字报告的人会漏掉最严重的那条。分诊一律走 `--json`。** ***
...
- 🆕 **E3（Web 面板）的计划已全部执行完（最近一轮 2026-09-15／16，Orca run `orca-dev-5d5c8055`）**：
  Task 0–9 各自实施、过审、由另一席在 clone 副本里跑完点名变异；最强模型做了整支终审（判 ready with fixes），一波修复已落地并复审复验。
  面板有列表／详情、记纠正（走 CLI 的 seam、**不闭环**）、显式「同意」、未审高位决策待办、Host 白名单（防 DNS rebinding），
  并有一条 Rule 4 意义上的端到端成功判据 `npm run verify:panel` 接进了 Orca 的 `npm run verify`。
  🆕 **那条 parked（N-1）已于 2026-09-16 修掉**（Orca run `orca-dev-cbe8dda3`，主题行
  `fix(web): take the previous decision off the screen before loading the next`）：切换行时先清旧详情，
  并新增一道守卫丢弃**乱序返回**的应答 —— 后者是写判据时才撞见的、parked note 没点名的第二个洞。
  ⚠️ *** **同一条 parked 点名的另一件事（给详情组件上 `key`）经实测判为冗余，没落地** *** —— 见下文第 7 条的正面用法。
  🆕 **又一轮（2026-09-16，Orca run `orca-dev-4ccc0a0b`，主题行
  `fix(panel): let the body parser's own status reach the client, and read brackets as an IP literal`）
  清掉三条登记挂账**：依赖漏洞分诊（**以「不升级 ＋ 一条离线机械判据」收口，不是以升级收口**）、
  body-parser 的 413/415 不再被压平成 400、`Host: [localhost]` 不再被当成合法名字。
  ⚠️ *** **这一轮里「先问再落地」第二次兑现，而这次答案是【红不了】** *** —— 见下文第 7 条。
  🆕 **又一轮（2026-09-16／17，Orca run `orca-dev-5e5985bc`）**：Orca 的 `reviews.jsonl` 有了留存策略 ——
  人手动 `orca compact-reviews [--apply]`（默认试跑）去重，并把**已归档**决策的行移进 `reviews-archive.jsonl`（孤儿要「归档里真有这个 id」的正向证据）；
  面板写入方在文件被替换后按文件身份重建去重集。⚠️ **与本仓库仍然无关**：写的是 Orca 的用户全局数据，不读 ccloop 的任何结构。
...
- 🆕 **子系统 D（检查点交接）有了设计草稿（2026-09-17，Orca run `orca-dev-bd5f202b`，Orca 主题行 `docs(spec): draft subsystem D checkpoint handoff…`）**：
  先服务**开发会话**，按上下文窗口水位（不是累计消耗）触发，目标是交接链本身不需要人；**D1＋D2 实施计划已落盘（2026-09-17，Orca run `orca-dev-8df1943a`，Orca 主题行 `docs(plan): turn D1 and D2 into seven tasks…`），尚未执行、无生产代码**。检查点住在**被干活的仓库**的 `.orca/checkpoints/` —— 本仓库不在 v1 范围内，**计划不碰本仓库**。
  ⚠️ *** **与本仓库的关系只有一条诊断（不是任务）**：D 把 ccloop 的阶段 agent **暂不纳入**，理由是 Orca 现测 ——
  每个阶段一个新 `claude -p --output-format json`（`scripts/claude-phase-runner.mjs`），结果只在进程结束时吐出；
  记的 token 是 input＋output、**不含 cache**（同文件 `USAGE_FIELDS`，行号引用前现测）⇒ 它不是窗口占用读数。 ***
  **重开条件**：实测到某个阶段的单次 `claude -p` 接近 D 的阈值。那时若要改 ccloop，照例先报、先拿具名授权。
  ⚠️ **Orca 从此有构建产物、根依赖是一棵树** —— 与本仓库仍然无关（Orca 通过 spawn 子进程用 ccloop，不是 npm 依赖）。
...
（§五 补上「子系统 D 有了设计草稿」及其对本仓库的一条诊断；两条新实测折进已有的第 4 条与第 6 条；**一个编号项都没有新增**）。
上一次是 run `orca-dev-18c77f3e` 于 **2026-09-17** 就地更新
（§五 E3 那一条补上「`reviews.jsonl` 非行不再让面板崩溃」；一条新实测折进已有的第 10 条；§七 过期的领先笔数改掉；**一个编号项都没有新增**）。
上一次是 run `orca-dev-5e5985bc` 于 **2026-09-17** 就地更新
（§五 E3 那一条补上「又一轮：reviews 压实」；两条新实测折进已有的第 7 条与第 10 条，**一个编号项都没有新增**）。
上一次是 run `orca-dev-4ccc0a0b` 于 **2026-09-16** 就地更新
（§五 E3 那一条补上「又一轮清掉三条登记挂账」；两条新实测分别折进已有的第 7 条与第 11 条，
**一个编号项都没有新增**）。上一次是 run `orca-dev-cbe8dda3` 于 **2026-09-16** 就地更新
（§五 E3 那一条改写成「parked N-1 已修，`key` 那一半经实测判为冗余」；一条新实测**折进已有的第 7 条**，
**一个编号项都没有新增** —— 规矩是人 2026-09-02 定的「关于 Orca 的章节不能无限增加下去」）。
上一次是 run `orca-dev-5d5c8055` 于 2026-09-16 更新
（§五 E3 改写成「计划全部执行完、留一条 parked」；三条新实测折进已有的第 3、7、11 条）。
...
*** **本地领先远端 1 笔、落后 0（run `orca-dev-8df1943a` 写入前现测；领先的是 run `orca-dev-bd5f202b` 那笔本节更新，未推）。** ***
