...
> ⚠️ **本节不写任何 HEAD、不写「领先几笔」、不写发布状态** —— 人会自己推远端。
> **要指代某一笔就引提交主题行；要判发布状态就现跑 `git ls-remote`。**
> ⚠️ 例外：**实测值的观测锚点必须写** —— 那是有效期，不是当前状态。
...
每轮开工与收尾都现测 `git status --porcelain` 为空。

## 三、本仓库可能想知道的（**都是诊断，不是任务**）

**观测锚点：ccloop 主题行 `docs(handoff): rewrite the Orca section in place -- orca correct's shape is settled…` 那一笔之后的第二次重写，2026-09-10 现测。**
...
1. **`refs/ccloop/<run-id>/attempts/<n>` 已经有真实消费者** —— Orca 的收产物层
   ...
   不是不许改，是**改之前要知道有人在读**。
   同理，契约的 `context.targetPaths` / `safetyPolicy.allowlistPaths`、`escalationAndExit.stopOn`
   与 `verification.stopSignals` 都是 Orca 判据的输入。
2. *** **`cancelled` 不需要给进程组发信号也能到达。** *** `runLoop.ts` 在 `evaluateStopDecision` **之前**
   ...
   ⇒ **一个 scripted frame ＋ `verifierType: "agent"` 就能跑出真的 `cancelled`。**
   ⚠️ 双方文档都曾写着「只能靠信号」，**那句不完整**；这条更正保留在此，别再写回去。
...
5. **`orca correct` 已实现并跑通过真闭环**（2026-09-07／08）。它是**人手动触发、会改目标仓库工作树并自己提交**的写入方：
   闭环模式取目标仓库的 repo 锁（`<repo>/.git/orca-lock`，**与 `orca run` 同一把**）、只提交它写出的那一个
   `.decisions/<run>.jsonl`、只记模式不取任何锁。提交前有一道**只读状态守卫**，拒绝处于 merge、cherry-pick、
   或 **detached HEAD** 的目标仓库；*** **`revert -n` 与 rebase 进行中【不拒】** ***。
   ⚠️ **今天与本仓库仍然无关**：本仓库不是 orca 的目标仓库。
   **登记的理由只有一条**：若将来本仓库成为目标仓库，那是一个会与 `orca run` 争同一把锁的新写入方。
6. **`orca metrics` 已实施**（2026-09-09）—— *** **纯只读：一个字节不写、一把锁不取。** ***
   它读各目标仓库的 `.decisions/**` 与一份全局 corrections store，算纠正率／修复率／积压。
   ⚠️ **今天同样与本仓库无关**（本仓库没有 `.decisions/`）。**登记的理由**：它是
   `refs/ccloop/**` 之外**第二个**会读目标仓库结构的下游，且它**默认只扫 `.decisions/` 顶层**、
   把 `archive/<YYYY>/` 留给定点查。
   🆕 **2026-09-10 补跑收口**：它的 **21 条变异现在条条被看见红过**（上一轮欠的 8 条已补齐，
   11 次单跑全部见红、无一跑绿）⇒ **没有发现新的真缺陷**，生产代码与判据都一行未改。

## 四、🆕 **与语言无关、值得本仓库直接抄走**的实测（本仓库也在做变异，用得上）

**观测锚点：Orca run `orca-dev-ad1e30c6`（第 1–5 条）、`orca-dev-3738db1c`（第 6–9 条）、`orca-dev-30fa920f`（🆕 第 10–11 条），2026-09-08 至 09-10。**

1. *** **`git clone --local` 只克隆【已提交】状态。** *** 要变异**未提交**的改动，必须先 `cat` 进副本并用 `diff`
   证明逐字节相同 —— 否则你变异的那个副本里**根本没有被测代码**。
2. *** **新克隆的副本没有 `node_modules`** *** ⇒ `npx vitest` 会拉一个错版本、死在 config 导入上，
   给你一个 `RC=1` —— **那不是判据变红，是启动错误**。
   ⇒ `ln -s <主仓库>/node_modules <副本>/node_modules`，再走 `./node_modules/.bin/vitest`。
3. *** **「绿」的一种坏法：判据对、变异也对，但两者之间隔着一条【时间上跑得比它慢】的断言。** ***
   实测：一个用挂钟算超时的锁，变异成注入时钟后 —— *** **「它拒绝了」那条断言【照绿】** ***（变异体真的跑满 1000ms）。
   红的是「注入的时钟被问了几次」那条。
   ⇒ **凡涉及时钟的判据，只断言「跑出来了／拒绝了」一定是空的。**
4. *** **一条判据可以在真实世界里造成它所测试的那个危害。** ***
   实测形状：一条守卫判据写成「绑 `0.0.0.0` 然后断言它没在监听」——
   **变异跑（守卫被删掉）时，它会真的在所有网卡上起一个能读写使用者全局数据的服务。**
   ⇒ **写守卫判据时问一句 —— 这条判据在【守卫被删掉】的那次跑里，会做出什么？**
5. *** **「本文未发布」是一条【会过期的现测】，不是文档的属性。** ***
   实测栽过一次：一份 spec 提交后 **2 分 29 秒**被人推上远端，随后两次「就地改」都拿开工时那次
   `ls-remote` 当依据 ⇒ **在已发布文本上就地改了两次，当时无人发现。**
   ⇒ **每次「就地改」之前现跑 `ls-remote` ＋ `merge-base --is-ancestor`；开工那次的结果不能复用。**
   ⚠️ **一处措辞更正**：`ls-remote` **只给远端 sha**；
   *** **要判「已发布文本有没有被就地改过」还要 `git diff <远端sha>..<本地sha> -- <文件>`。** *** 别把前者说成「唯一检测手段」。
   🆕 **2026-09-10 又兑现一次，方向相反**：上一轮收尾写着「6 笔未推、等人授权」，
   **下一轮开工现测发现人已经全推上去了** ⇒ 整包变成已发布文本，当轮的更正因此**只能追加 ERRATUM**。
   *** **「未发布」会过期成「已发布」，反向同样成立 —— 两个方向都必须现测。** ***
6. *** **计划／文档里写下「应当红／应当绿」，就是写下一条【预言】，而预言会是假的。** ***
   Orca 实测两条：「变异 X 跑完另一条应仍绿」（实际必红）、「期望红在断言 A」（实际到不了 ——
   前面那条断言先炸）。⇒ *** **两条都会【主动把执行者引向错误结论】** ***，
   因为纪律是「红在别处＝假红」，执行者会回去改没坏的代码。
   ⇒ **每写一条「期望红在 X」，把那条变异在脑内跑到底，问「在 X 之前有没有别的断言会先炸」。**
7. *** **一条变异要能红，先问「被删掉的那件事，是不是【唯一】在做它的那件事」。** ***
   Orca 实测：一条变异**落上去了（sha 变了）却照绿** —— 守卫冗余，被另一处逻辑遮住；
   顺着查才发现被遮住的是**一个真的缺陷**。
   ⚠️ **本仓库铁律里那句「redundant guard cannot be pinned by a mutation that deletes only itself」与它同形**，
   这条实测是它的第一份正面证据。
8. *** **计划正文里的 shell 是自审结构上看不见的那一块。** ***
   ...
   *** **shell 状态不跨调用持久** *** ⇒ 四处全部展开成 `/bin/rm -rf .`。
   ...
   (b) *** **有没有 `<` 或 `>` 出现在不是重定向的位置** *** —— 现测占位符文件存在时会造出名为 `--as-of` 的文件。
   **(a) 结构上看不见 (b)，因为那里根本没有变量。**
9. *** **扫描器不许对自己的警告文字报警。** *** Orca 同一轮踩了两次：
   一个 shell 扫描器命中了它自己那句「别用 `<…>`」的注释；一条纯度判据命中了它自己那句「不许有 `new Date()`」的注释。
   ⇒ **一律先剥注释再判。** *** **一个对自己注释报警的扫描器，会被读的人学会忽略。** ***
10. 🆕 *** **「期望红在 X」写得【不全】，和写得【假】是同一类危害。** ***
    第 6 条防的是**假**预言。Orca 2026-09-10 实测出同一形状的第二种：**真但不全** ——
    ...
    **危险在于**：纪律是「红在别处 ＝ 假红」，看到「说红 1 条、实际红 2 条」最省事的读法就是判它假红，
    *** **然后回去改没坏的代码** *** —— 与第 6 条引出的是同一个错误动作。
    ⇒ **两条一起用**：问「X 之前有没有别的断言会先炸」（防假）＋
    🆕 **问「被删掉的那一行，还有【谁】在走它」（防不全）**。**写成「红在 X 且仅 X」或「红在 X 与 Y」，不留「至少」。**
    ⚠️ *** **它与第 7 条是同一个问句的两个方向** *** —— 第 7 条问共用会不会让变异**变绿**，
    本条问共用会不会让它**红得比写的多**。**一次「谁还在走这条路」的普查，同时回答两个问题。**
11. 🆕 *** **守着那条铁律的扫描器，自己带着假阳性被交付了。** ***
    Orca 把它计划里那个 Q1 扫描器（第 8 条那道最后防线）原样跑了一遍，它报一条 `BAD`，
    **而那一行完全正确** —— 用了 `/bin/rm`、每个实参都带 `:?`、变量都在同块赋值。
    **错的是扫描器**：它把变量名**写死成了 `T`**，任何不叫 `T` 的正确写法都会被报。
    ⚠️ 这与第 9 条同族，后果一样：*** **读的人学会忽略它，而一个被忽略的护栏等于没有护栏。** ***
    ⇒ **扫描器要按【护栏本身】判，不认变量名。**
    🔴 **更要紧的是修它的过程**：修正版**自己又错了两次**（`^` 不认 `A=x; B=y` 的第二个；补完又漏了 `re.M`），
    **两次都是靠【反向对照】抓住的，不是靠读代码。**
    ⇒ *** **一个扫描器只在语料上跑是不够的，必须同时有【必抓】和【必不抓】两组样本。** ***
    *** **「在本文上跑出 BAD_COUNT = 0」什么都不证明 —— 一个恒返回 0 的扫描器给同样的输出。** ***

## 五、Orca 那边到哪了（**知情，不复述细节**）

- **A′（决策台账校验器）、子系统 C（调度层）已落地；子系统 B 的入口条件已做完。**
- **子系统 E 的第一刀（`orca correct`）已实施完毕并通过验收**，`overturned` 的形状就此冻住。
- *** **E 的第二刀（E2：指标核心 `orca metrics`）已实施完毕并收口**（实施 2026-09-09，变异补齐 2026-09-10）***：
  spec ＋ 计划各经外派评审，**两席各判一次 `Ready to implement? No`**，处置后实施；
  🆕 **21 条变异现已条条见红**，且**没有发现新的真缺陷**。
- **E3（Web 面板）已有定稿 spec，尚未实施**；⚠️ 它有一件**需人另拿具名授权**的开工前置，**至今未拿到**。
- **E4（DB）不出 spec** —— 由一条**实测阈值**触发（现测全量扫 Orca 台账 144 行 ＝ 2.4 ms）。
...
| | 角色 |
|---|---|
| **ccloop（本仓库）** | **一个工具** —— 把单个任务跑成循环。**Orca 通过 spawn 子进程使用它，不是 npm 依赖** |
| **Orca**（`/Users/biran/code/skills/loop/Orca`） | **系统** —— 调度、决策台账、指标核心、Web 面板 |
...
写入时**现测**本仓库：在 `main`、`git status --porcelain -z` 输出 0 字节、与远端同点。
