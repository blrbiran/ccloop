...
架构真相源仍是 Orca `docs/superpowers/specs/2026-09-19-task-control-design.md` §13。控制底座八任务、Codex 五任务、公共协议／handoff／D3 八任务均完成，勿重复实施。Orca Web 可恢复控制计划的 **Task 1–10 已全部提交在其本地 `main`**（定位只用主题：Task 9 `feat(web): add recoverable task control`，Task 10 `test(control): verify web recoverable control`，其生产改动另起 `fix(panel): serve the evidence bytes the manifest hands to the browser`；审查修复仍见 `fix(control): close Task 6 settlement review gaps`、`fix(control): reject noncanonical validated estimates`）。Task 8 给下游的契约未变：handoff-stop 与 panel shutdown 都靠 `control handoff` ＋ `control inspect` 把冻结集合收口（pause 只挡新 claim，不触碰已接受 run），`beginHandoffAttempt` 每次现探测能力并把 proof 身份钉在 attempt ordinal 上，`failed-before-provider` 的 re-arm 复用同一 continuation intent 身份只推进 ordinal；改这些字段或 `collect`／`read-evidence` 语义要同步核对 Orca consumer。Task 10 用真 ccloop 子进程（`/tmp/ccloop-codex-0919/dist/cli.js`，无外部模型）钉住三件跨仓事实：`control capabilities` 仍由 `src/control/command.ts:133-143` 硬编码成 `usageObservation: phase-end`、`budgetEnforcement: soft`、`requestBoundEvidence: null`，所以 Orca 的 strict 组一律 `control-capability-unsupported`、开不出 run；V1 envelope 之外的输入（`protocol: 2`、`generation: 0`、越出 `sourceDir/input` 的 `bundlePath`）一律 exit 2 且稳定映射成 `control-peer-exit:2:<code>`；被台账冻结的 dispatch envelope 能被真进程 durable accept 一次、重放同结果。跨仓缺口也在这轮现测确认：Orca 生产侧**没有**把台账 `orca-dispatch-envelope-v1` 翻成 ccloop `StartEnvelopeV1` 的 consumer（翻译只在 Orca 的测试文件里），且 `createCcloopExecutionPort` 不暴露 `probeProfileCapabilities`，裸 ccloop port 驱动不了 Web dispatch。ccloop 本轮零改动。Orca 那条线在本节写完后又跑完整支终审与一波修复（三笔本地提交，主题：`fix(control): book a continuation against what its predecessor did not spend`、`fix(web): keep an unresolved command id and open run evidence with the token`、`docs(sdd): append the whole-branch corrections to the web control report`），其中只有第一笔动控制台账、且**没有改本节列出的任何跨仓契约**（`control handoff`／`control inspect` 收口、ordinal、`collect`／`read-evidence` 语义原样）。终审同时更正了 Orca 验收报告里九处读不回证据的说法，包括旧的「1519 全通过、零 skip」——本轮没有 `/tmp` ccloop artifact 时能实测到的是 `npm test` RC0＝173 文件通过／1 skip、1520 测试通过／5 skip，typecheck 与 Web 门（13 文件／64 测试）干净，正式 ccloop 门本轮未重跑。发布状态不在本文判定：`git status -sb`、`git log --oneline @{u}..HEAD`、`git ls-remote` 现测。下一位从 Orca 那三笔修复之后的挂账接手，再依次自动拆分、ccmem 纠正闭环／组 goal 验收；Web > CLI。

## awaitingHuman 与边界

分支整合已在本地完成，是否已推送不在本文判定（用户曾自行推送，发布状态一律现测：`git status -sb` ＋ `git log --oneline @{u}..HEAD` ＋ `git ls-remote`）；分支/worktree 未删除，开发树、证据和未入库的 node_modules 软链保留，不绕闸门。Orca Web 计划 Task 1–6 的复现与实现报告在 Orca 开发树 `.superpowers/sdd/2026-09-20-web-recoverable-control/`（gitignored，作为历史证据保留、一字不改），Task 7–10 的台账、RED／GREEN 表、验收映射、未过滤日志与 API 证据在 Orca main 树 `.superpowers/sdd/2026-09-19-web-recoverable-control/`（`progress.md`／`commands/`／`acceptance-map.md`／`test-logs/`／`artifacts/`／`final-report.md`，已随 Task 10 提交）。下一位从 Orca `docs/superpowers/plans/2026-09-20-web-recoverable-control.md` 的 Final completion checklist 接手（十项任务本体勿重做）。
