...
1. **待人裁（不要替人裁）**：`SweepOptions.stderr` 旁**没有任何「must not throw」契约注释**。那条「`appendEvent` 吞、回调不吞」的不对称论证，依据已从**结构性**（回调不做 I/O）退成**约定性**（落点是调用方注入的槽）。今天生产侧只有一个注入点（`src/cli.ts:234` 的 `console.error`），**三方独立确认无实际缺陷**。选项：写成显式契约 / 重新论证这条不对称 / 维持现状并记债。
2. **一处保留的冲突（按 Rule 7 不折中）**：`spec:2322` 该不该跟改 —— 实施者与 scoped 再评审员**两方独立深读**后都判「不改」并各自给了理由（目的性短语 vs 定义性断言），整分支评审记为 Minor。**现状是不改。**
3. **10 条 deferred minor**（9 条延后 ＋ 1 条新增：全仓另有两个同形文件 L1 的 spec/plan，文首同样带修订总账，**本分支从未碰过，其自身是否过期从未验证**）。逐条在包 3 台账。
