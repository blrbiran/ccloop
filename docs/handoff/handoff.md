...
- 🆕 **E3（Web 面板）在做（最近一轮 2026-09-11／15，Orca run `orca-dev-5d7759dc`）**：
  ...
  **Task 0／1／2／3 已实施并各自过审**（`web/` workspace ＋ express ＋ `orca panel` 骨架；reviews 表自带一把 mkdir 锁、
  自己的拒绝码与显式 mode；安全边界 —— `--by` 必填、token 常时比较、非回环绑定要第二句确认），**Task 4–9 未做**。
...
（§五 E3 那一条改写成「Task 0–3 已实施过审、Task 4–9 未做」；三条新实测**折进已有的第 4、6 条**，
**一个编号项都没有新增** —— 规矩是人 2026-09-02 定的「关于 Orca 的章节不能无限增加下去」）。
上一次是 run `orca-dev-6354277a` 于 2026-09-11 更新（E3 改写成「已开工」，两条实测折进第 6、11 条）；
再上一次是 run `orca-dev-3e50f2cb` 于 2026-09-10 更新（更正「React 零命中」，两条记法折进第 4、10 条）。
写入前**现测**本仓库：在 `main`、`git status --porcelain` 输出为空、
*** **`git ls-remote` 与本地【不同点】——本地领先一笔（主题行 `docs(handoff): roll the Orca section forward -- E3 has started, and two readings that were wrong`），那是上一轮写的，至今未推。** ***
⚠️ 这一句只在写下的那一秒为真，**人随时会自己推；要判发布状态就现跑 `ls-remote`。**
*** **本次只改本文档一个文件、且只改本节**；`src/**`、`tests/**`、`scripts/**`、`.superpowers/**` 零触碰。 ***
**节外内容（本文档第 1–313 行）的 sha256 在改写前后逐字相同**，这是「没碰到别人的东西」的证明。
