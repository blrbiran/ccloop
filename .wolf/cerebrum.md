...
- A-04 approval truthfulness needs a separate repo-root main-checkout baseline (`rev-parse HEAD` plus full `git status --porcelain`) captured before prepare starts; a dirty start or any later drift must reject approval even though deterministic verification runs in a preserved isolated checkout.
