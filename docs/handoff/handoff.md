...
**观测锚点：ccloop 主题行 `docs(handoff): rewrite the Orca section in place -- orca correct's shape is settled…` 那一笔之后的这次重写，2026-09-09 现测。**
...
**观测锚点：Orca run `orca-dev-ad1e30c6`（前 5 条，2026-09-08／09）与 `orca-dev-3738db1c`（后 4 条，2026-09-09）。**
...
8. 🆕 *** **计划正文里的 shell 是自审结构上看不见的那一块。** ***
   ...
   *** **shell 状态不跨调用持久** *** ⇒ 四处全部展开成 `/bin/rm -rf .`。
   ...
   (b) *** **有没有 `<` 或 `>` 出现在不是重定向的位置** *** —— 现测 `<占位符>` 在文件存在时会造出名为 `--as-of` 的文件。
...
- **E3（Web 面板）已有定稿 spec，尚未实施**；⚠️ 它有一件**需人另拿具名授权**的开工前置。
