...
# 📌 Orca 那条线的进度更新（**本节由 Orca 那边的会话追加，2026-08-29；上面一字未动**）

*** **对本仓库的影响仍然是零。本节只是知情，不产生任何任务**，也不改变
E1 的 I-2 ＋ 人裁 85 那一轮的任何安排。 ***

## 先更正上面两处已经不成立的话（**原文逐字保留，此处即为具名更正**）

**1. 「本会话对本仓库的改动一览」写的「都只在本地提交，一次没 push」—— 现在为假。**

本节写下时现测：`git ls-remote origin refs/heads/main` 与本地 `main` **同点**，
且那三笔（README、spec 迁出、铁律迁入 `CLAUDE.md`）**逐笔都是远端 tip 的祖先**
（`git merge-base --is-ancestor` 现测，三笔全部返回 0）。

*** **推的人是人类，手动推的。控制器一次都没 push —— 那条铁律没有被破。** ***

⇒ *** **`CLAUDE.md`（含 Rule 13–18）现在是【已发布文本】。** *** 改它要按已发布文本对待。

**2. 「对本仓库的影响：目前为零」那节写于 spec 刚落地时。**

spec 现已多出**一份实施计划，也已发布**。**影响仍然是零**，但理由变了 ——
从「它还只是设计」变成「**设计与计划都明确把本仓库排除在外**」。

## Orca 那边现在到哪了

`docs/superpowers/plans/2026-08-29-decision-ledger-validator.md`（**在 Orca 仓库，不在本仓库**）
—— A′ 决策台账**校验器**的实施计划，8 个任务，**尚未开始执行**。**Orca 至今零产品代码。**

## 🔴 本仓库【真正需要知道】的一件事：两个契约字段现在是承重的

Orca 的并行判据是（spec §5.1）：

> **写集 ＝ `context.targetPaths` ∪ `safetyPolicy.allowlistPaths`；两个任务写集相交 ⇒ 必须串行。**

这两个字段（`src/contract/schema.ts`）当初是为**安全**加的。它们现在**多了一个下游消费者**。

⚠️ ⇒ *** **改这两个字段的名字或语义，会静默弄坏 Orca 的调度判据。** ***
不是「不许改」，是 **改之前要知道有人在读它**。而且坏法是**延迟且静默**的 ——
Orca 以 npm 依赖方式引入本仓库并**锁版本**（spec §9.1），
**版本没升上去之前，本仓库这边的改动根本不会传过去**，等升版本时才一起爆。

同类的还有两处，**都只是引用，不构成任何约束**：

- Orca 的台账写入方**照抄** `src/persistence/fileStore.ts` 里 `appendEvent` 的形状
  （`appendFile(..., JSON.stringify(event) + "\n")`）—— 这是 Rule 11 conformance，不是接口契约。
- Orca 的 run-id **复用本仓库 run 目录的 basename**（spec §3.0）
  ⇒ *** **run 目录名从此是一个【身份】，不只是一个路径。** ***

## Orca 那边一条可能对本仓库有用的实测（**建议，不是任务**）

Orca 那轮实测到 **`rtk proxy git log --oneline -N` 会漏笔** ——
输出以第二新的那笔开头，**HEAD 那一笔整个不见了**，裸 `git log` 能看到。
本仓库「踩过的坑」一节记的是别的几种工具骗法，**这一种此前没记过**。

⚠️ *** **要不要写进本仓库的坑清单，由本仓库的下一轮自己决定。本节不代为修改任何既有小节。** ***

## 边界（**没变**）

Orca 的规则**不放松本仓库的任何铁律**（spec §7）。
E1 的 I-2 ＋ 人裁 85 那一轮 **仍完整跑在 `CLAUDE.md` Rule 13–18 之下**。
*** **本节不构成对本仓库的任何授权，也不是任务。`src/**` 与 `tests/**` 本轮一个字节都没动。** ***

---

# 📌 Orca 那条线：A′ 的校验器已全部落地（**本节由 Orca 那边的会话追加，2026-09-01／02；上面一字未动**）

*** **对本仓库仍然【零任务】。本节是知情 ＋ 一条对本仓库铁律 6 的实测警告。** ***
E1 的 I-2 ＋ 人裁 85 那一轮的安排**一点没变**，仍是本仓库的下一件事。
本节写下时**未触碰** `src/**`、`tests/**`、`.superpowers/**`，也未动上面任何一节。

## 🔴 一条对**本仓库铁律 6** 的实测警告（**这是本节唯一需要本仓库动脑的地方**）

铁律 6 写着：**「还原证明看 `git diff` 与 `git diff --cached` 的字节数」**。
Orca 那边本轮实测到：

> *** **`git diff` 对【未跟踪文件】的内容改动完全看不见。** *** 新建一个未跟踪文件，`git diff | wc -c` 是 0；
> 把它整个覆写掉，**前后仍然都是 0 字节**，`git status --porcelain` 打印同一行 `??`。

⇒ **只要被变异的文件在那一刻还没被 `git add` 过，铁律 6 的那个证明就什么也没证。**
本仓库历轮的变异对象大多是**已提交**的红线函数，所以历史上没踩到；
但**新增文件的那种变异**（例如给某模块加一个新文件再变异它）会静默落进这个洞。

*** **建议**（不是任务，需人拍板）：把「还原证明」的口径从 `git diff` 的字节数
改成对参与文件取 `shasum -a 256` 前后比对 —— 它看得见未跟踪文件的内容。 ***

同一轮还实测到两条相关的：

1. *** **`git checkout -- <尚未提交的新文件>` 报 `error: pathspec ... did not match any file(s) known to git`，
   exit=1，什么都不还原。** *** 拿它当还原手段会静默失效。
2. *** **`git checkout -- <path>` 是从【索引】恢复，不是从 HEAD。** *** 文件已 `git add` 过时，
   它会把**暂存的那份**写回工作树 —— 如果暂存的正是坏内容，这一步等于把坏内容又装回去。

## Orca 那边现在到哪了（**知情**）

那份 8 任务的计划已**全部执行完毕** ＋ 一轮整支复审的修复。
*** **「Orca 至今零产品代码」这句话不再为真。** *** 它现在有 spec §3.8 六项检查的实现、一个 CLI、
一个 fail-closed 的写入方，和一道 pre-commit 闸门。**Orca 那边一次都没 push。**

进度与细节见 `/Users/biran/code/skills/loop/Orca/docs/handoff/handoff.md` 的最后一节 ——
**本节不复述它**。

## 本仓库那两个契约字段：**仍然承重，且已现测仍在**

`src/contract/schema.ts` 的 `context.targetPaths` 与 `safetyPolicy.allowlistPaths`
（本节写下时现测：两者都还在，分别在该文件的 `targetPaths: z.array(z.string()).min(1)` 与
`allowlistPaths: z.array(z.string()).default([])` 两行）。

Orca 的并行判据是**两者取并集，相交则必须串行**（Orca spec §5.1）。
⚠️ **改这两个字段的名字或语义会静默弄坏 Orca 的调度判据**，而且坏法是**延迟的** ——
Orca 以 npm 依赖锁版本引入本仓库，**版本没升上去之前改动不会传过去**，等升版本时才一起爆。
**不是不许改，是改之前要知道有人在读它。**

⚠️ 注意：**调度器（子系统 C）本身还没开工**，所以这条约束目前是「登记在案」，不是「已经有代码在跑」。

## 边界（**没变**）

Orca 的规则**不放松本仓库的任何铁律**（Orca spec §7）。本节**不构成对本仓库的任何授权，也不是任务**。
