...
## 实施期间的更正（**2026-09-23，追加，上文一字不改**）

1. **Global Constraints §8 的「已知红 7 条」不全 —— 现测是 12 条。**
   补进的 5 条与判别证据见 spec §11.1。**引用名单前先确认它是哪一轮测的。**
2. **Task 的落地顺序是错的**：Task 7（逃逸点）必须先于 Task 4 的第二条判据，
   实际执行顺序为 **3 → 7 → 4 → 5 → 6**。见 spec §11.4。
3. **Task 3 的 M3-2／M3-3 红证预言是错的** —— 「由 Task 4／5 的判据接住」实测为零红，
   已补两条专钉闸门的判据。见 spec §11.3。
4. **示例夹具与期望理由自相矛盾**（`pid:0` 的锁配 `EPERM` 的断言）。见 spec §11.5。
5. **变异副本一律建在会话 scratchpad 目录下**，不在仓库旁边（人裁 137）。
