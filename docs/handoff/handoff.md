...
> 此后每一轮都再明确一次：**「关于 Orca 的章节不能无限增加下去」**。
...
5. **`orca correct` 已实现并跑通过真闭环**（2026-09-07／08）。它是**人手动触发、会改目标仓库工作树并自己提交**的写入方：
   ...
   或 **detached HEAD** 的目标仓库；*** **`revert -n` 与 rebase 进行中【不拒】** ***（实测过度拒绝被有意避免）。
   ⚠️ **今天与本仓库仍然无关**：本仓库不是 orca 的目标仓库，也没有 `.decisions/`。
   **登记的理由只有一条**：若将来本仓库成为目标仓库，那是一个会与 `orca run` 争同一把锁的新写入方。

## 四、🆕 五条**与语言无关、值得本仓库直接抄走**的实测（本仓库也在做变异，用得上）

**观测锚点：Orca run `orca-dev-ad1e30c6`，2026-09-08／09。**

1. *** **`git clone --local` 只克隆【已提交】状态。** *** 要变异**未提交**的改动，必须先 `cat` 进副本并用 `diff`
   证明逐字节相同 —— 否则你变异的那个副本里**根本没有被测代码**，跑出来的红全是假的。
...
3. 🆕 *** **「绿」的又一种坏法：判据对、变异也对，但两者之间隔着一条【时间上跑得比它慢】的断言。** ***
   实测：一个用挂钟算超时的锁，变异成注入时钟后 —— *** **「它拒绝了」那条断言【照绿】** ***，
   因为变异体真的跑满 1000ms 才超时（1006ms，对比绿时整个文件 199ms）。红的是「注入的时钟被问了几次」那条（收到 `[]`）。
   ⇒ **凡是涉及时钟的判据，只断言「跑出来了／拒绝了」一定是空的 —— 要断言【哪个时钟被问了】或【时长的字面值】。**
4. 🆕 *** **一条判据可以在真实世界里造成它所测试的那个危害。** ***
   实测形状：一条守卫判据写成「绑 `0.0.0.0` 然后断言它没在监听」——
   **变异跑（守卫被删掉）时，它会真的在所有网卡上起一个能读写使用者全局数据的服务。**
   ⚠️ **历轮纪律都在防「判据【碰】真实用户数据」，这是新的一种：判据【制造】真实攻击面。**
   ⇒ **新记法：写守卫判据时问一句 —— 这条判据在【守卫被删掉】的那次跑里，会做出什么？**
5. 🆕 *** **「本文未发布」是一条【会过期的现测】，不是文档的属性。** ***
   Orca 那边实测栽过一次：一份 spec 提交后 **2 分 29 秒**被人推上远端，而随后两次「就地改」
   都拿开工时那次 `ls-remote` 当依据，**违反了注释铁律而当时无人发现**。
   ⇒ **每一次「就地改」之前现跑一次 `ls-remote` ＋ `merge-base --is-ancestor`；开工那次的结果不能复用。**
   ⇒ ⚠️ *** **收尾的 `ls-remote` 不只是报「还剩几笔没推」——它是「我这一轮有没有改过已发布文本」的唯一检测手段。** ***
   **本仓库铁律 5 与它同形，这条实测是它的第一份反面证据。**
...
- 🆕 **E 的后续已出【两份】spec 并定稿**：**E2**（指标核心，纯只读）与 **E3**（Web 面板）。
  各经一轮自审 ＋ 一席外派评审，**各被整份重写一次**；两席都判 `Ready to implement? No`。
  **DB（原计划的索引层）被推迟成「由一条实测阈值触发」** —— 现测全量扫 Orca 台账 **144 行 ＝ 2.4 ms**。
- **两份 spec 都还没有实施计划**（`writing-plans` 未跑）。
