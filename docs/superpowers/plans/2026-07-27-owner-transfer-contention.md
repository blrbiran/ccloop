...
- ~~**No terminal-state changes.** No exit code, no run status, no `persistTerminalState` call moves.~~ **Amended 2026-07-28 (a), matching spec §2's amendment of the same date: this constraint was wrong, and it is a defect in this plan and in the spec it inherited it from — not in the implementation.** `persistBoundaryAnalysis`'s guards precede terminal persistence, and that is what *causes* a refusal to change the run state rather than preventing it: the throw reaches `isLeaseStopError` and persists `"cancelled"` / `"lease_lost"` before the outcome the attempt was heading for is ever written. Two tests now assert exactly that, on the human ruling that authorized the unconditional write guard. What still holds: **no exit code changes** (nothing in `src/cli.ts` maps a run status to a process exit code) and **no `persistTerminalState` call site moves**.
