...
34. 人裁 100 —— 收口：I-3b 与 M-3 两处补完，注释轮到此为止
--------------------------------------------------------------------------------

*** **人裁 100。2026-08-25。「修 I-3b ＋ M-3，然后收口 —— 不再为注释修复派第三轮评审。」** ***

控制器向人陈述的收口理由（人采纳）：**「为修复派评审」会无限递归** —— 每一轮修复都在制造新的未评审面，
今天已完整走了一遍（评审 → 修 → 修出来的三笔又没人审）。连续两轮独立评审均 **0 Critical**，
且第二轮的三条 Important **全部是注释文字准确性，无一涉及行为**。继续递归买不到安全，只买字面完美。

### M-3 的现场（控制器自己核，评审员那条只是 read-only argument）

| | |
|---|---|
| 病灶 | 人裁 83 的 ERRATUM 把「永久滞留集合」描述为 `every lock that is not a parsed pid:<n> whose process is dead` |
| 读者会读成 | **活 `pid:<n>` 持有的锁也在集合内** —— 但它不滞留，持有者退出即释放 |
| 更糟的一句 | 同一 ERRATUM 说这让 `--force` 覆盖了更多格 |
| 实测反证 | `src/unlock/unlockCommand.ts` 的 alive 分支**拒绝并故意不打印 `--force` 那行**；同文件二十行上明写 **"A LIVE HOLDER IS NEVER REMOVED, --force INCLUDED"**（人裁 70 board C-e） |
| 后果 | 操作员照该句去敲 `--force`，会被**拒第二次** |
| 两处 | `src/unlock/unlockCommand.ts` 与 `tests/unlock/inspectLock.test.ts`，**同一句话的两份拷贝** |

### 铁律适用的分界（**本条是先例**）

- **人裁 98 的就地改**只适用于**本会话自己刚写、从未为真**的笔误（I-1）。
- I-3b 与 M-3 这三段是**上一会话写的、且已在远端历史里** ⇒ **铁律不打折**：原文逐字保留，追加具名 ERRATUM。

### 落地与实测（未过滤整份读回）

提交主题行 `docs(unlock): close I-3b and both M-3 sites …`

| 项 | 值 |
|---|---|
| diff | **+28／−0**（三文件 10／10／8），**非注释改动 0 行**，**删除 0 行** |
| 最宽注释行 | `inspectLock.ts` 100／`unlockCommand.ts` 101／`inspectLock.test.ts` 101 —— **改前改后逐一相同** |
| 红线函数 | **本轮逐字节未动**（`git diff --numstat` 对该文件 0 字节），基线仍 **3185** |
| 全套件 | **`34 files / 602 tests`** 全绿零 skipped，`TEST_RC=0`，`RUN` 路径 = 主仓库根 |
| typecheck／build | `0`／`0` |

### ⛔ 下一件事

1. *** **只剩一件在人手上：点 B 是否通过、C-1 是否记关闭、E1 是否通过。控制器一件都没宣布。** ***
   人裁 96 那轮评审的全部 Important 已闭合（I-1／I-2／I-3a 在 §33，I-3b 在本条）。
2. **仍开着的 Minor（8 条，人裁 100 明确不再为它们派评审）**：M-1（C 提交信息记错文件与字符数）、
   M-2（`e22d1ea` 甩出去的那句仍在 ERRATUM 之后）、M-4（`open(lockPath,"wx")`）、M-5（A 缺正向观测）、
   M-6（T2 注释只挂在较弱那条上）、M-7（一处 freeze 主张只补了方向 erratum）、M-8（pid namespace，与本轮无关）、
   M-9（无需动）。**M-3 已在本条闭合。**
...
37. M-1 的处置被推翻 —— 「只记台账」不成立；以及一次不该发生的成本自估
--------------------------------------------------------------------------------

**不是新裁决，是人裁 104 之下的处置更正。** 人的指令：「修复查出来的问题，然后进行独立评审。」

### §36 把 M-1 判成「只记台账」，这个判断是错的

§36 的理由是「提交信息在远端头上，改不了」。**那句话对，但它不构成处置。**
*** **M-1 所指的那笔提交（`docs(comments): make e22d1ea's own method claim true …`）自己就写下了反驳：** ***

> The fix is the code, not a ledger footnote. A commit message travels with `git log`; a note in the
> ledger does not, so leaving the false account in place and explaining it elsewhere would reproduce
> the defect shape this package exists to remove — an integrity claim with nothing enforcing it.

⇒ **把这条原则用在它自己的提交信息上**，就是本节做的事。按铁律 4，**§36 原文一字不改**，更正记在此处。

### 更正落在哪里，以及为什么只落一处

| | |
|---|---|
| 落点 | `tests/sweep/sweepRuns.test.ts`，紧接既有 ERRATUM 之后 |
| 为什么是这里 | **顺着那句假话找过去的读者，唯一会落到的地方就是这个文件** |
| 为什么 `fileStore.ts` 不加 | *** **那边不含任何假陈述。** *** 坑 3 的「半改比不改坏」讲的是**同一句话散在两处**（M-4 就是那种），本条不是那个形状 —— 无差别撒网会把 erratum 变成噪音 |

⚠️ **提交信息本身永远是假的**，这一点没有变，也没法变。变的是：**读者现在会在落地点上撞见实测值。**

### 一次不该发生的成本自估（**记下来当规矩**）

控制器在上一段收尾时向人报「本轮约 $6」，并把这个数写进了 handoff 的预算段。
**成本钩子随后报出的会话总额是 ~$57。** *** **那个 $6 从未为真 —— 它是凭感觉估的，不是从任何工具读来的。** ***

- 该数是**本会话自己刚写、且从未为真**的笔误 ⇒ 按人裁 98／100 立的分界，**就地改**，不追 ERRATUM。已改。
- ⇒ *** **新规矩：成本只报工具给出的数。拿不到数就说拿不到，不许自估。** ***
  这与本包的主题同形：**一个没有东西在支撑的数字，就是本包存在的理由那种缺陷。**

### 落地与实测（未过滤整份读回）

提交主题行 `docs(comments): put M-1's correction where a misled reader lands, not only in the ledger (human ruling 104)`

| 项 | 值 |
|---|---|
| diff | **+11／−0**，**删除 0 行**，非注释改动 0 行 |
| 最宽注释行 | `tests/sweep/sweepRuns.test.ts` **101** —— **改前改后相同**（本轮口径：全文件、首个非空白为 `//`、按字符计） |
| 新增行最宽 | **97** |
| 全套件 | **`34 files / 603 tests`** 全绿零 skipped，`TEST_RC=0`，耗时 18.32s |
| typecheck／build | `0`／`0` |
| 红线函数 | **本节逐字节未动**，基线仍 **3185** |

### ⛔ 下一件事

1. *** **人要的第二件：为本会话的成果派独立评审。** *** 被审面 = 本会话**五笔代码／注释提交 ＋ 两笔台账 ＋ 两笔 handoff**。
   ⚠️ brief 抄 `…/pointB-cleanup-review-brief.md`，**其中三处必须按现测更新**：
   判据基线 **603**、红线函数 **3185 字节**、已知 flake **四条**（人裁 10 名单外那两条必须写上，否则评审员会当真回归去追）。
2. ⚠️ **人裁 100 的收口理由在此【不适用】** —— 那条针对的是「为修复反复派评审」的递归；**本轮是人主动指名要审**，是人裁，不是控制器自己加轮。
3. **控制器全程未 push。**

38. 人裁 105 —— 独立评审回来了；控制器在 §36 里对上一会话的指控是假的
--------------------------------------------------------------------------------

*** **人裁 105。2026-08-25。「修复查出来的问题，进行独立评审。」** *** 评审回来后追认：**「全修」**（5 条 Important ＋ 6 条 Minor ＋ 控制器自己那条假指控）。
*** **人同时确认：远端是人自己推的。** ***

### ⚠️ 先记最重的一条：**§36 的第二个「记账缺陷」是假的，而且是控制器自己的错，已经推到远端**

§36 写下：「§33／§34 的『最宽注释行』口径从来没被写死，三种全文件口径都复现不出来，差一且张冠李戴」。
*** **这是假的。真相是控制器的量尺坏了。** *** macOS 的 `awk` `length()` **按字节计**，`LC_ALL` 对它无效 ——
控制器用 `LC_ALL=en_US.UTF-8 awk` 量出的「字符数」**全部是字节数**。

用真正的字符实现重量（`git show <rev>:<path>` ＋ python `len()`）：

| 文件 | §33／§34 记的 | 实测（字符，全文件最宽注释行） | @`060fbbf` | @`06c6e31` | @HEAD |
|---|---|---|---|---|---|
| `src/persistence/fileStore.ts` | 101 | **101** | 101 | 101 | 101 |
| `tests/persistence/fileStore.test.ts` | 103 | **103** | 103 | 103 | 103 |
| `src/unlock/inspectLock.ts` | 100 | **100** | 100 | 100 | 100 |
| `src/unlock/unlockCommand.ts` | 101 | **101** | 101 | 101 | 101 |
| `tests/unlock/inspectLock.test.ts` | 101 | **101** | 101 | 101 | 101 |

*** **五个数，五个全中。** *** §33／§34 用的一直是最显然的口径 —— **全文件、最宽注释行、按字符** —— 从头到尾可复现。

⇒ **§36 那条「记账缺陷」不存在。存在的是控制器的坏探针，以及在探针没被验证的情况下对前人工作发出的指控。**
⇒ **§36 立的那个「新口径」从来就不需要**，而且它自己也没被遵守（本轮全部数字都是字节数贴了字符标签，评审员的 I-3 抓到了这一半）。
⇒ 该指控已随 `0801920`（台账）与 `809bb50`（handoff）**推到远端**。按铁律 4，**§36 原文一字不改**；更正在此。
⇒ *** **规矩：一个探针在被验证之前，它的输出不是证据。这与 §25.20 那个「本机无容器运行时」的坏探针同形 —— 第二次了。** ***

⚠️ **评审员没抓到这一层**，因为 brief 里那句「别跟 §33／§34 比，§36 记了原因」把它挡住了 ——
*** **控制器自己的错误经由 brief 变成了评审员的前提。** *** brief 已追加具名 ERRATUM，原文逐字保留。

### 评审结论：**0 Critical／5 Important／6 Minor**

被审面 = `62a4d49`／`d905592`／`f6fbf01`。评审员的协议合规：读-only 已证（主仓库 `git status --porcelain`／`git diff`／`git diff --cached` 收尾均 0 字节），
变异只在 `git clone --local` 副本、已 `/bin/rm -rf`，每条发现都标了 measured／read-only argument，全套件独立复测 **603 确认**。

### 控制器逐条复核（**不接受自陈**）

| | 复核结果 |
|---|---|
| **I-1** | ✅ 成立。`C-1 is not recorded as closed` 还有 **2 处**：`src/persistence/fileStore.ts`（ruling 83 那条 erratum 的末句）与 `tests/persistence/fileStore.test.ts`（并发读者块的 ERRATUM 3）。**两处都在本轮编辑过的文件里**，其中一处距本轮新加的 erratum 只有 26 行 |
| **I-2** | ✅ 成立。`…/pointB-review.md` 的 `## Critical` 下第一条就是 C-1 ⇒ **第一轮有 1 条 Critical，不是 0** |
| **I-3** | ✅ 成立，**且根因比它说的更深**（见上）。真值：101／103／99 字符 |
| **I-4** | ✅ 成立。`git ls-remote` 现跑：`origin/main` = `809bb50`，本轮**前四笔已在远端** |
| **I-5** | ✅ 成立。第三处现在时 `open(lockPath, "wx")` 在 `describe` 块的**标题行**，正是本轮改过其内部注释的那个块，而**读者先读到标题** |
| **Mi-1** | ✅ 成立。插入 M-7 erratum 把「that mutation」的先行词推远了一块 —— **修 M-7 制造了一个 M-2 形状** |
| **Mi-2** | ✅ 成立。「and the function changed」那半句只在 2 处，不是全部；但操作性主张（freeze-lifted 那句）确实全部都有 |
| **Mi-3** | ✅ 成立，**实测可证**（见下） |
| **Mi-4** | ✅ 成立。变异 B 下该文件红 **4 条**而非 1 条；M-5 的具体主张不受影响（另三条钉的是「回收能力」丢失，没有一条观测到**测试 A 自己的夹具**进没进函数），但提交信息的措辞会让读者以为该变异别处不可见。**提交信息在远端，改不了 ⇒ 只记此处** |
| **Mi-5** | 评审员自陈「不需要再加 erratum」，控制器同意：M-2 的 erratum 已经明写了意图先行词 ⇒ **只记此处** |
| **Mi-6** | 采纳为规矩：**erratum 不得引入新的、会被后续裁决推翻的计数**。本轮全部新 erratum 一律指向台账，不写数 |

### Mi-3 的变异证明（副本内，主仓库工作树只读）

| 变异 | 计数测试（`observes that the redline function actually ran…`） | **新结构测试** |
|---|---|---|
| 往 `acquireOwnerTransferLock` 加一条无害的 `await readFile(lockPath, "utf8")` | *** **全绿 —— 它分辨不出** *** | *** **红：`expected [ Array(2) ] to have a length of 1 but got 2`** *** |

⇒ **与 M-5 同形，只是这次被空转的是 M-5 自己的前提。** 新判据用 TS 编译器解析而非正则（照
`tests/controller/ownedRunStateWriter.structure.test.ts` 的先例），带 must-hit／must-miss 反空转锚点，
并在断言上写明**它不覆盖什么**（首参不是 `lockPath` 标识符、经由别的 API 读、别的模块读）。

### 落地（两笔）

1. `docs(comments): close every site the Minors round left standing (I-1, I-2, I-4, I-5, Mi-1, Mi-2, human ruling 105)` —— **+51／−0**，三文件
2. `test(fileStore): enforce the "one reader" premise the counting test rests on (Mi-3, human ruling 105)` —— **+92／−0**，新文件

⚠️ *** **前四笔已在远端 ⇒ 本轮全部更正一律追加 ERRATUM，就地改 0 处。** ***

### 落地后实测（未过滤整份读回）

| 项 | 值 |
|---|---|
| 全套件 | *** **`35 files / 604 tests`** *** 全绿零 skipped，`TEST_RC=0`，`RUN` 路径 = 主仓库根，耗时 21.92s |
| typecheck／build | `0`／`0` |
| 新结构测试单跑 | `1 passed`，**27ms**，`RC=0` |
| 最宽注释行 | **101／103／99 字符**（`fileStore.ts`／`fileStore.test.ts`／`sweepRuns.test.ts`）—— **改前改后逐一相同** |
| 红线函数 | **本轮逐字节未动**，基线仍 **3185**，签名命中数 =1 |

*** **判据基线自本条起是 604，测试文件数 35。口径回到 §33／§34 一直在用的那个：全文件、最宽注释行、按字符。** ***

### ⛔ 下一件事

1. *** **本轮无人裁待决。** *** 评审的 5 条 Important ＋ 6 条 Minor 全部处置完毕（Mi-4／Mi-5 记录不改，Mi-6 转为规矩）。
2. ⚠️ **远端 `origin/main` = `809bb50`，本地领先 4 笔**（`f6fbf01`／`713a87e`／`979f5f0` ＋ 本轮两笔 ＋ 本节）。
   **控制器仍不许 push**；要不要推是人的事。
3. **仍挂账、需要人先开口的三件不变**：I-3（红线函数返回类型，建议新会话 brainstorming）、E1 的 I-2 那一格（授权面外）、Linux（OrbStack daemon 要人自己起）。
4. ⚠️ **本轮这两笔自身又没经过独立评审。** 人裁 100 的递归收口理由适用，但**要不要再破一次例是人的事，未裁**。

39. 人裁 106 —— 三件同开：I-3、给上一轮两笔再派独立评审、记台账收工
--------------------------------------------------------------------------------

*** **人裁 106。2026-08-25。** *** 人在 §38「⛔ 下一件事」列的挂账里**同时点开三件**：
**(a) 开 I-3**（让红线函数把「为什么返回 false」告诉调用方）、
**(b) 给上一轮那两笔再派一次独立评审**、
**(c) 记台账 §39 收工**。
*** **E1 的 I-2 那一格与 Linux 覆盖【仍未开】，维持挂账。** ***

⇒ *** **(a) 是对红线函数的【新授权】。** *** 人裁 50 的封印曾被人裁 83 部分解除，但 §27 已写明
「人裁 83 之后对该函数的改动由人裁 83 授权，且**仅限**人裁 83 那段措辞所描述的改动」——
I-3 要改的是**返回类型**，不在人裁 83 的措辞内 ⇒ **授权来自本条，不得再往外扩。**
⇒ *** **(b) 明示不适用人裁 100 的收口理由。** *** 与人裁 105 同形：**人主动指名要审时，人裁 100 不适用。**

### 开工基线复核（本会话现跑，未过滤整份读回）

| 项 | 实测 | 判定 |
|---|---|---|
| `git ls-remote origin refs/heads/main` | 与 §38 记的**不同** | *** **远端未被再推动；本地领先 1 笔**（§38 记的「远端领先 4 笔」那句已过期，不再引用其中的哈希）*** |
| 全套件 | *** **`35 files / 604 tests`** *** 全绿零 skipped，`TEST_RC=0`，耗时 **19.99s**，`RUN` = 主仓库根 | 与 §38 基线**一致** |
| typecheck／build | `0`／`0` | 一致 |
| 红线函数签名命中数 | **1**（`src/persistence/fileStore.ts:970`，返回 `Promise<boolean>`） | 一致 |

### 把「3185」的口径钉死（**§38 只记了数字没记口径，本会话为此跑了一趟死胡同**）

`tryRecoverStaleOwnerTransferLock` 的 **3185 字节**，量的是：
*** **`src/persistence/fileStore.ts` 第 970–1025 行的整行范围，含第 1025 行末尾那个换行符**（`sed -n '970,1025p' | wc -c`）。 ***

用另一个同样合理的口径量同一段——「从 `async function` 起、大括号配对到收尾 `}` 为止」的切片——得 **3184**。
*** **差的正是那一个换行符；函数本身逐字节未动。** *** 两个数都对，缺的是口径。

⇒ 这是 handoff「踩过的坑」第 5 条（*** **报任何数字，连同口径一起报** ***）的**第三次复发**
（前两次：M-1 的三个数三方都对、`awk length()` 数字节被当成数字符并因此发出过一条假指控）。
⇒ **§38 原文一字不改，更正落在本节。** 此后引用红线函数字节基线，**必须连口径一起引**。

### ⛔ 下一件事

1. **I-3** —— 按人裁 106(a) 开工，用 `superpowers:brainstorming` 起手（**有实质设计成分，不许直接写码**）。
   控制器的既有建议仍在：与人裁 85 那轮（`ls` 也报锁）是**同一个病**，值得一起想。
2. **独立评审** —— 按人裁 106(b)，brief 需**同时覆盖**「上一轮那两笔」与「I-3 本轮改动」，一次审完。
...
43. 独立评审（人裁 120）＋ 修复轮（人裁 123／124）—— 三处吞里有一处从来没被钉住，是评审查出来的
--------------------------------------------------------------------------------

*** **人裁 120。2026-08-28。「本轮四笔派独立评审。」** *** （人裁 110 只对人裁 108 那一笔收口，不覆盖本轮。）
*** **人裁 121。同日。「挂账里先动 E1 的 I-2 ＋ 人裁 85；Linux 继续挂。」** ***
*** **人裁 122。同日。「修完派复审。」** *** （本轮有 1 Critical，人裁 100 的「连续两轮 0 Critical」前提不成立。）
*** **人裁 123。同日。「C-1 只补一条判据，不做人裁 118 式披露注释。」** ***
*** **人裁 124。同日。「I-1 整条改写；I-2／I-3 向 N1 各加一条断言；K-1 追加具名 ERRATUM —— 四条全授权。」** ***

材料：brief `.superpowers/sdd/2026-08-07-pkg2-data-loss/i3a-review-brief.md`、
报告 `.superpowers/sdd/2026-08-07-pkg2-data-loss/i3a-review.md`。
派评审时的范围是 `30dde52..d4a9bb1`（八笔）—— 开工时先取成 `565aaae`，**自查发现那会把人裁 108 那一笔卷进来**
（人裁 110 已收口），改正后才派。

### 评审结论：**1 Critical／3 Important／5 Minor**；判 Not ready as it stands

⚠️ *** **评审员没在台账／spec／plan 里查出任何一处假数** *** —— 613、4769（连口径）、红线函数逐字节未动
（它自己按签名＋大括号配对重新定位后测的，并比对了 `30dde52` 与 HEAD 的 sha256）、消费点普查、
M1／M3／M4／M5／M6／M8 全部独立复现，结论与 §42 一致。**生产代码本身没查出错，缺的是证明。**

### 控制器逐条复核（**四条承重主张全部自己跑，一条自陈都没照收**）

| | 主张 | 控制器的复核 | 结论 |
|---|---|---|---|
| **C-1**（Critical） | 人裁 113 的 `stop()` 记录分支无判据钉住 | 副本里删掉整段 ⇒ **612 passed / 1 failed**，唯一红的是已知 flake `records env names only…`，**单跑该文件 39 全绿** | 成立（实测） |
| **I-1** | 改写把生产调用排到了断言之后 | 调出 `30dde52` 的版本逐行对比：改写前 `const owner = await readOwnerRecord(...)` **在**锁内容断言之前，改写后挪到了之后 | 成立（**只读即证，不需要变异**） |
| **I-2** | N1 只钉两个否定，钉不住「原地」 | 副本里让该分支写入终态 `exhausted`（`legalTransitions.exhausted = []`）⇒ **613/613 全绿，RC=0** | 成立（实测） |
| **I-3** | M6 下 N1 照绿 | 副本里撤回收窄 ⇒ 红的**恰好**是那四条改写判据，**N1／N2／N3／N4 全绿** | 成立（实测） |

### *** K-1 —— 评审员没发现，控制器自己查出来的：一笔提交里两处注释互相打脸 ***

同一笔 `fix(runLoop): route a blocked transfer recovery to abandonment…` 里：
`runLoop.ts` 的注释老实写着「M8 删掉这行，下面那条判据照绿 —— 本包没有任何东西钉住这次写入」（人裁 118），
而它给 N1 写的注释却说 **「Mutation M8 is what proves this pair is load-bearing」**。
**M8 从没红过 ⇒ 测试侧那句是假话，而且写下它的那一刻就是假的。**
按注释铁律（上一会话写的、且未推远端）：**原文逐字保留 ＋ 追加具名 ERRATUM**，不就地改。

### 落地（三笔，按文件切开，均为本地提交）

1. `test(fileStore): restore the weight the ruling-111 rewrite took out of one assertion …`
2. `test(runLoop): pin the outcome and the path this criterion only implied …`（含 K-1 的 ERRATUM）
3. `test(leaseHeartbeat): pin the release-path record that nothing was pinning …`

**生产代码一行未改。** 红线函数仍是第 1017–1095 行、**4769 字节**
（口径＝`src/persistence/fileStore.ts` 整行范围、含末尾换行；现测，签名命中数 1）。

### 变异证明（四条，全部**看见打红**；`git clone --local` 副本，主仓库工作树全程零触碰）

⚠️ 本轮改动**未提交时**就要证，所以副本是「clone 已提交状态 ＋ `cat 工作树文件 > 副本对应文件`」，
并先证明六个文件与工作树 **sha256 全等**才开跑；副本的「已还原」基线是 `git diff` = **7821 字节**
（＝那三个判据文件的改动本身），不是 0。

| | 变异 | 实测 |
|---|---|---|
| C-1 | 删掉 `stop()` 整个记录分支 | **新判据红**：`expected [] to have a length of 1`（改前：全套照绿） |
| I-1 | 在两个 unattributable 出口都加 `safeUnlink(lockPath)` | `strong instance-id` 那条**红在锁内容断言那一行**（改前评审员实测它绿，其余同批红）⇒ 红的条数 5 → **6** |
| I-2 | 该分支改写终态 `exhausted` | **N1 红**：`expected 'exhausted' to be 'executing'`（改前：613/613 全绿） |
| I-3 | 撤回收窄（M6） | **N1 红**：`expected 'owner transfer abandoned: …' to contain 'recovery blocked'` —— 红出来的字面就证明那条事件是**转移路径**发的 |

还原后逐一比对：副本 `git diff` 回到 7821 字节、`git diff --cached` = 0 字节，
六个文件与工作树 **sha256 全等**，然后 `/bin/rm -rf` 删除。主仓库 `porcelain`／`diff`／`diff --cached` 全程可查。

### Mi-1 的补记（**§42 少说了一处**）

人裁 115 第一条改写（`keeps a malformed lock non-recoverable even when staged artifacts are present`）
删掉的不止 `expect(owner.currentOwnerEpoch).toBe(1)` 一条断言，**还有 `expect(owner.currentProcessInstanceId).toBe("pid:12345")`**。
两条都因为「读不再返回记录」而无法保留，删除本身站得住；**但 §42 与 spec §3.2 只交代了前一条。**
（铁律 4：§42 原文一字不改，更正落在本节。）

### 落地后实测（未过滤整份读回）

*** **`35 files / 614 tests`** *** 全绿**零 skipped**（613 ＋ C-1 那条新判据），
`TEST_RC=0`／`TYPECHECK_RC=0`／`BUILD_RC=0`，耗时 16.84s，vitest 第一行 `RUN` 指向本仓库。
*** **判据基线自本节起是 614。613 作废。** ***

### *** 本轮的方法论：「没跑过的那条变异」也不是证据 ***

§42 记下的教训是「红在哪条断言不可靠」。本轮补上它的反面：
**八条变异看着完备，是因为每条新增分支都有一条以它命名的变异 —— 唯独 `stop()` 那一支只被别的分支的变异间接掠过。**
⇒ **机械检查：每新增一个分支，点名那条删掉【它自己】的变异，并确认它存在。**
⚠️ 还有一条更钝的：*** **改写判据时，断言的【位置】和断言的【文字】一样是承重的。** ***
I-1 那处三条断言一字未改，只是顺序变了，其中一条就此不再观测任何生产行为 ——
**「逐字保留」不等于「承重保留」，验收改写要看的是「它还能不能红」。**

### ⛔ 下一件事

1. *** **派复审（人裁 122，已裁）。** *** brief 抄 `…/i3a-review-brief.md` 改：范围是本节这三笔，
   基线 **614**，红线函数连口径，已知 flake 写满 4 条，并把 plan 的**第二次更正节**一并给评审员。
   评审面只盯修复：新判据能不能被变异打红、I-1 重排后是否真的承重、有没有第五条被动过的判据。
2. **挂账（人裁 121 已开口两件）**：**E1 的 I-2**（要设计，**动生产代码前须另拿具名授权 —— E1 在授权面外**）、
   **人裁 85**（`ls` 也报锁）。**Linux 仍挂着**（OrbStack daemon 要人自己起）。
3. **控制器不许 push。** 本节三笔与前面七笔全部只在本地。

44. 复审（人裁 122）—— 四条修复都被打红过；**并且：本包已全部推到远端，注释规则的适用面自此变了**
--------------------------------------------------------------------------------

材料：brief `.superpowers/sdd/2026-08-07-pkg2-data-loss/i3a-rereview-brief.md`、
报告 `…/i3a-rereview.md`。范围 `d4a9bb1..26da28e`（三笔判据 ＋ 两笔文档）。

### 评审结论：**0 Critical／1 Important／4 Minor**

*** **它没在两处 ERRATUM、新判据注释、台账 §43、plan 第二次更正节里查出任何一句假话。** ***
它专门去打这一面（「本轮存在的理由就是一句已发布的假话」），逐句复跑：M8 现在仍绿（614/614）、
「改前绿改后红」它**把 BASE 版本的判据文件签出到副本里重跑了一遍**才认、`src/` 与配置文件 diff 0 字节、
人裁 115 那四条的名字它自己从 `5365da1` 的删除行反查出来、§43 记的副本基线 7821 字节也复现。

### 四条修复的红证（复审员自己跑的，与控制器各自独立）

| 修复 | 变异 | BASE（改前） | HEAD（改后） |
|---|---|---|---|
| C-1 | 删掉 `stop()` 整个记录分支 | 全套照绿 | **新判据红** |
| I-1 | 两个 unattributable 出口都加 `safeUnlink` | 5 红，那条**绿** | **6 红**，那条**红在锁内容断言本身** |
| I-2 | 该分支写终态 `exhausted` | 29/29 **绿** | **N1 红** |
| I-3 | 撤回收窄（M6） | 29/29 **绿** | **N1 红**，且红出转移路径的原文 |

*** **它还替新判据补了一条控制器没做的攻击**：把释放分支的 detail 改成 affirm 分支的措辞 ⇒ **新判据红在 detail 断言**
⇒ 证明「断言 detail 而不只是类型」这一步本身是承重的，不是装饰。 ***

### *** 本轮的机械检查，回头套到整个 I-3(a) 轮上，现在通过了 ***

「每新增一个分支，点名那条删掉【它自己】的变异」—— 五个新增分支（收窄、`resumeLoop` 病名、`runLoop` 放弃、
心跳 affirm、心跳 release）各有一条以它命名的变异，**复审员五条全部跑过并看见红**。
**本轮之前，最后一行是空的。** 这正是 C-1 能溜过上一轮的原因。

⇒ **再补一条更便宜的、不用变异就能查的子规则**（复审员提的，成立）：
*** **一条排在被测调用【之前】、读回测试自己刚写进去的值的断言，永远不可能红。** *** 验收改写时直接扫这个形状。

### ⚠️ Important —— **本包已全部推到远端，三份文档因此变成假话**

*** **控制器现测：`git ls-remote origin` = `26da28e`，与本地 HEAD 相同；`.git/logs/refs/remotes/origin/main`
记着三次 push：`809bb50 → 4493338 → d4a9bb1 → 26da28e`。人在修复轮提交后约四分钟推的。** ***

⇒ **§43 末尾那句「本节三笔与前面七笔全部只在本地」当时为真，现在为假。**
（铁律 4：§43 原文一字不改，更正落在本节。）handoff 是活文档，已就地改正。

⇒ *** **后果不是记账，是规则切换**：本轮那两处 `ERRATUM (I-3(a) FIX ROUND, HUMAN RULING 124)` 与新判据的注释
**自此是已发布文本** —— 将来若被实测推翻，唯一合法的修法是**再追加一条具名 ERRATUM**，不许就地改。 ***

### 四条 Minor 的处置（**控制器逐条自己核过，不是照收**）

| | 内容 | 处置 |
|---|---|---|
| Mi-1 | `stop()` 分支里 `unattributableLockRecorded = true` 是**死写**，删掉全套照绿 | **成立，只读即证**：`stop()` 幂等（`if (stopped) return;` ＋ `stopped = true`），`runAffirm` 在 `stopped` 时直接返回 ⇒ 没有任何东西能再观测该标志。**不补判据**（没有可观测行为可钉），记在此处即为披露 |
| Mi-2 | 三处新注释用「above」定位，靠位置而非名字 | **成立，不动**：改它要动已发布文本，代价是再加一条 ERRATUM，买不到真值。**下一轮写新注释时点名邻居，别写「above」** |
| Mi-3 | I-1 的 ERRATUM 末句「Nothing was added, removed or weakened」字面上不成立（那一段加了十行注释） | **成立但只是措辞**（作用域显然是断言，断言集合确实一字未改）。不动，同上 |
| Mi-4 | 上一轮的 Mi-3（N4 名字过度承诺）与 Mi-5（新事件类型无消费者）仍开着 | 按 brief 判为出范围（前者要改既有判据的名字，须人另行指名；后者是人裁 85 的账）。复审员确认**没有新后果** |

### 落地后实测（未过滤整份读回，本节写下时）

*** **`35 files / 614 tests`** *** 全绿零 skipped，三个 RC 全 0；红线函数仍第 1017–1095 行、**4769 字节**
（口径＝整行范围含末尾换行），复审员用 sha256 比对了 `30dde52`／`d4a9bb1`／HEAD 三点，逐字节相同。

### ⛔ 下一件事

1. *** **本轮 0 Critical，四条修复各有红证，两轮评审都没查出假数。要不要就此收口是人的事 —— 未裁。** ***
   ⚠️ 人裁 100 的原文前提是「**连续两轮** 0 Critical」，上一轮有 1 Critical，**严格说还不满足**。
2. **人裁 121 已开口两件**：**E1 的 I-2**、**人裁 85（`ls` 也报锁）**。两件都有实质设计成分 ⇒ **先 brainstorming**。
   ⚠️ *** **E1 在授权面外：动生产代码前必须另拿一次具名授权。** ***
3. **Linux 仍挂着**（OrbStack daemon 要人自己起）。
4. **控制器不许 push。** 现在远端与本地同点，**新写的任何一笔又只在本地**。

45. 人裁 125／126 —— I-3(a) 收口，本会话到此为止
--------------------------------------------------------------------------------

*** **人裁 125。2026-08-28。「I-3(a) 这一轮收口。」** ***
⚠️ **这是人主动开的一个例，不是援引人裁 100** —— 人裁 100 原文要求「连续两轮 0 Critical」，
本轮首审有 1 Critical（C-1），**严格说不满足**；§44 把这一点摆出来之后人仍然拍了收口。
⇒ **以后引用时要引人裁 125 本身，不要说成「人裁 100 的适用」。**

*** **人裁 126。同日。「本会话停在这里，E1 的 I-2 与人裁 85 的设计另开会话。」** ***
⇒ 人裁 121 开口的那两件**仍然有效，只是不在本会话做**。

### 本会话（2026-08-28）总账

- 做完：I-3(a) 轮的**独立评审**（人裁 120）→ **四条修复**（人裁 123／124）→ **复审**（人裁 122）→ **收口**（人裁 125）。
- 提交六笔：三笔判据 ＋ 三笔文档（§43、handoff 更正、§44）。**生产代码一行未改。**
- 判据基线 **613 → 614**；红线函数近三轮一个字未动（1017–1095 行 / 4769 字节，口径＝整行含末尾换行）。
- 远端：本会话开工时 `4493338`，人推了两次，收尾时 `26da28e`；**§44 之后新写的文档笔只在本地**。

### ⛔ 下一件事（**下一个会话的第一件事**）

1. **E1 的 I-2**（人裁 121 已开口）：先 `superpowers:brainstorming` 出 spec。
   ⚠️ *** **E1 在授权面外 —— 出完设计、动生产代码之前，必须另拿一次具名授权。** ***
2. **人裁 85 —— `ls` 也报锁**（人裁 121 已开口）：同样先 brainstorming。
