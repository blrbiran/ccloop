...
⚠️ **`:282` 差一行**（实为 283；282 是 `): Promise<ReconciliationRecord> {`）。**这是先于本分支就有的错**——`fileStore.ts` 在 `07180a7..HEAD` 字节未变，不是被本分支顶掉的，按 Rule 3 不改数字，就地勘误。定位用 `grep -nF 'async function preserveSuccessfulReconciliationIfNeeded(' src/persistence/fileStore.ts`（`-F` 不可省，理由见下一段）。

**论据已更正。** 初稿给的理由是「loser 的转移根本没进入事务」——**那条从未验证过**，而且与 staging/CAS 的先后有关，本记录无权断言。真正的理由不依赖事务，而且强得多：`persistOwnerTransfer` 的返回类型 `Promise<{ ownerRecord: OwnerRecord; eligibleForContinuation: true }>` 就把 `eligibleForContinuation` 钉成**字面量 `true`**，函数末尾的 `return` 兑现。所以赢家**必然**命中 `async function preserveSuccessfulReconciliationIfNeeded(` 的早退（**锚点带前缀是必需的**：裸符号名在 `fileStore.ts` 里命中 4 行，其中 **2 行**指向另一个函数 `preserveSuccessfulReconciliationIfNeededFromArtifacts`——定义 `:222`、调用 `:292`；另 2 行才是本函数——定义 `:279`、调用 `:312`。带 `async function ` 前缀才唯一。重推用 **`grep -nF`**，`-F` 不可省：本仓库的 `grep` 被改写成正则引擎，锚点末尾的 `(` 会被当成未闭合分组而**报错退出 2**，而不是返回 0 命中），loser（`eligibleForContinuation` 保持 `runLoop.ts` 里 `let eligibleForContinuation = false;` 的初值）**必然**不命中。**这是类型级保证，与事务机制无关。** 两者不相交。
