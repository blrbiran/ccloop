...
⚠️ *** **上一会话在这件事上栽了两次**：报告「一笔未 push」时，远端已被人推进四笔；收尾复核之后、写本文之前，远端又被推了一次。
**`ls-remote` 的结果只在它跑出来的那一秒为真 —— 开工一次、收尾一次，中间做过判断就再来一次。** ***
...
### 上一会话（2026-08-25 第二段）实测基线 —— **开工与收尾各测一次，两次一致**；未过滤整份读回、`RUN` 路径已核
...
⚠️ *** **写本文时，上面这些提交已【全部发布】** *** —— `git ls-remote` 与本地一致，领先 0 笔。
但**提交本文这个动作本身就会让本地再领先一笔**，而且人随时会再推。
⇒ *** **开工第一件事是自己现跑 `git ls-remote`，别信这一句。** ***
⇒ *** **凡已发布的文本，一律只能追加具名 ERRATUM，没有就地改的余地** *** —— 上一会话的更正全部是这么做的（就地改 0 处）。
...
上一会话**约 $106**（**工具报数**，含一名独立评审员 **168k tokens**），**远超 CLAUDE.md Rule 6 的每会话 400k**。
