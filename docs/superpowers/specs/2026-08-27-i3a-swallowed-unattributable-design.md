...
## 7. 落地更正（**上文一字未改；本节是实测对本设计的更正**）

本节记录设计里被**实测推翻或修正**的说法。上面 §1–§6 原文逐字保留，读上文时以本节为准。

### 7.1 §2.3 的次序表述不准（M2 实测）

上文写「唯一承重的次序是「在通用失败处理之前」——**落在其后就等于不存在**」。
**后半句是错的。** M2 实测：把该支移到通用失败处理之后，它**仍然执行、事件照记**，红的是状态断言
（`expected 'failed' not to be 'failed'`）—— 因为通用失败处理不 `return`，而是落到函数末尾统一 `return state`。
准确的说法是：**排在其后，事件照记，但 `failed` 已经先落定**。承重的仍是那个次序，只是失效方式不同。

### 7.2 §2.3 未量前提 1 —— **已量到，成立**

无该支时：`status = "failed"`，事件流 `[loop_planning, attempt_started, execute_started, attempt_failed]`。
一次被锁挡住的恢复确实会被升级成一次失败的尝试。设计里那句「四处注释这么说、但本轮探针量不到」自本节起有了自己的证据。

⚠️ **度量方式也要更正**：设计原以为「N1 先红在 `not.toBe("failed")`」就能证明这条前提。**不能。**
N1 的事件断言排在状态断言之前，会先短路。实际靠的是一条**定向探针**（副本里把状态打印出来）。
⇒ **「红在哪条断言」不是可靠的判别方式**，要量什么就直接量什么。

### 7.3 §2.3 未量前提 2 —— **量了，没被钉住**（人裁 118）

M8 只删 `writeOwnedRunState` 那一行，N1 **照绿**：在 N1 走的那条路上，返回的 `state` 与盘上的本来就一致。
⇒ 该行**没有任何判据承重**。人裁 118：**留着**（与 `RunHeartbeatStoppedError` 那一支同形），
并在代码注释里写明「理由是继承来的，不是这里证明的」。设计原文按同形定为「写」，方向对，但把它当成了已论证的，实际不是。

### 7.4 心跳的事件类型 —— **人裁 119 推翻了 §2.3 的先例外推**

§2.3 为 `runLoop` 那一支写了「故意不新造事件类型」，**实施时把这条先例外推到了心跳**，用了同一个
`owner_transfer_contended`。**实测打红两条在数该类型条数的判据**（一条是人裁 106 立的既有判据，计数 1 → 2）。
⇒ 人裁 119：**心跳用自己的类型 `owner_transfer_lock_unattributable`**。既有判据一字未动。

**这次碰撞本身就是证据**：消费者确实在数这些事件，所以「转移被放弃」与「续租被挡」不可互换。
§2.3 那条先例对 `runLoop` 仍然成立（那里确实是一次转移被放弃），**它只是不能外推到心跳**。

### 7.5 §4 变异表的实测结果

M1／M2／M3／M4／M5／M6／M7 **全部见红**；**M8 未红**（见 §7.3）。
M4 另有一条值得留下的观察：**删掉心跳 affirm 那一支后，N4 仍绿**——因为 `stop()` 那一支照记一条。
⇒ 「记了」与「只记一次」必须是两条判据，合成一条会被另一半掩盖。
