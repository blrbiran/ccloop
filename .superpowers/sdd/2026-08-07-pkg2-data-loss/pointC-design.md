...
> *** **已裁 —— 人裁 70（2026-08-19）。本节保留为「当时问了什么」的记录，结论见 §7。** ***
...
## 7. 人裁 70 —— 六块板已裁（2026-08-19）

*** **人裁 70。2026-08-19。「同意，按你的建议走」——控制器 §5 的六条建议全部被采纳。** ***

| 板 | 裁决 | 落地要点 |
|---|---|---|
| **C-a** | *** **走第三条路：presence-only** *** —— 既不进 `OBSERVED_FILES`，也不在 `sweepRuns` 本体里读文件 | 只探**存在性**，**不 parse、不取身份、不判存活** ⇒ 不产生第二套 JSON 读取实现 ⇒ **spec §7.2 不被触发、人裁 50 的红线不被碰**。探测放进**注入的 dep**（与 `scan` 同形），`sweepRuns` 本体仍不读任何文件 ⇒ §3 #1 也不必推翻 |
| **C-b** | **两种形态都报**，同一个循环、同一句话 | 报告是**盘上状态的属性**，不是候选资格的属性。遍历 `rows`，不是 `candidates` |
| **C-c** | **不加** `RUN_MARKER_FILES` | 只有锁、别无他物的目录按 §1 分析基本不可达；为发生不了的情形改 spec §4 识别语义是纯风险。**且本轮未测过它** |
| **C-d** | **fail-closed ＋ 打出拒绝理由**；真要删给 `--force`，且要求人把 holder id 敲进去 | 把「判断锁死没死」**整个移出代码**，代码只陈述事实。⚠️ 见下方「关于 C-d 的诚实附注」 |
| **C-e** | **必须带测试**，按五种锁状态逐一钉住，尤其钉死**活 pid 绝不删** | 实测：新增删锁命令 535/535 全绿、**零测试拦它** |
| **C-f** | *** **E4 划掉** *** | §4.3 端到端否证已成立 |

**顺序（一并裁）**：*** **E3 先单独落一轮；E1 另起一轮。** ***
E3 是只读、纯新增；E1 开的是**删除**面，按仓库既有做法要走
「实施者 → 换人评审 → 修复环 → 再换人 scoped 评审」。两件混一轮，评审注意力会被 E3 的零风险稀释掉 E1 的真风险。
...
(c) 撞红多少测试。**不要把「推出来的」当成「测出来的」用。**

### 关于 C-d 的诚实附注（下一位读这段再动 `unlock`）

控制器在提建议时**明确请人亲自定 C-d、并说了「不要采纳我的默认」**；
人的回复是**整体性的「同意，按你的建议走」**，因此 C-d 是**随整体采纳落定的，不是单独选过的**。
控制器据此按已裁处理，但**在开工实现删锁面之前，建议向人复核一次 C-d**，把控制器当时陈述的代价原样摆出来：
*** **fail-closed 的代价是——坏锁（读不懂）恰恰是运维最需要这条命令的场景，而它在那里拒绝。**
`--force` 是兜底，但那等于承认逃生口需要人读懂现场再敲一次命令。
若人更看重「一条命令自己脱困」，C-d 应当反过来，且必须接受它与 C-1 的判据分歧被写进两个方向。 ***

### 另一件事：范围之外，已获整体同意但建议单独确认

**锁记录 holder 形如 `^pid:\d+$` 的钉桩测试**（把 `fileStore.ts:724-726` 那条「do not unify」注释升级成被强制的不变量）。
