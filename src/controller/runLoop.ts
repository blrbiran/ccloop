...
// Package 2 / debt D-1, task S4: the run-state writer lives in its own module so that this one has
// no import of `writeRunState` to call. Pinned by tests/controller/ownedRunStateWriter.structure.test.ts.
import { createOwnedRunStateWriter } from "./ownedRunStateWriter.js";
import type { OwnedRunStateWriter } from "./ownedRunStateWriter.js";
import {
...
// F-1) and the guard has moved to createOwnedRunStateWriter, which task S4 in turn moved out of
// this module into ./ownedRunStateWriter.ts — see the erratum there. What
